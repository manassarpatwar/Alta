...
get'/userOrders' do
    ...
    if params[:column].nil? && !params[:allAlltype] && !params[:paidtype] && !params[:freetype] && !params[:cancelledtype]
        ...
        # @results = $db.execute query
        @results = $db.execute('SELECT * FROM journeys WHERE user_id = ' + session[:id] + ' AND date_time = ' + params[:search]) 
        ...
        # @results = $db.execute query
        @results = $db.execute("SELECT * FROM journeys WHERE user_id = '#{session[:id]}' AND start_location = '#{params[:search].strip}'") 
        ...
        # @results = $db.execute query
        @results = $db.execute('SELECT * FROM journeys WHERE user_id = ' + session[:id] + ' AND end_location = ' + params[:search]) 
        ...
        # @results = $db.execute query
        @results = $db.execute('SELECT * FROM journeys WHERE user_id = ' + session[:id] + ' AND free_ride  ' + params[:search]) 
